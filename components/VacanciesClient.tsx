...
export default function VacanciesClient() {
  ...
  const [collapsed, setCollapsed] = useState<Record<string, boolean>>(
    () => Object.fromEntries(ALL_CATS.map((c) => [c, false]))
  );
  ...
  // Urgent jobs that match current filters — shown as a pinned section at the top
      ...
      <div className="w-full lg:min-w-0 lg:flex-1 lg:max-w-2xl">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            {/* ── Urgent / Priority Vacancies (pinned at top) ──────── */}
            {urgentJobs.length > 0 && (
              <div className="rounded-2xl border border-red-500/25 bg-red-500/[0.03] overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3.5">
                  <div className="flex items-center gap-2">
                    <span className="text-red-400 text-[11px] font-black uppercase tracking-widest">
                      🔴 Urgent · Priority Vacancies
                    </span>
                    <span className="text-[10px] font-bold bg-red-400/10 text-red-400 rounded-full px-2 py-0.5">
                      {urgentJobs.length}
                    </span>
                  </div>
                </div>
                <div className="flex flex-col gap-1.5 px-3 pb-3">
                  {urgentJobs.map((job) => (
                    <div
                      key={`urgent-${job.slug}`}
                      className="rounded-xl border border-white/[0.08] bg-white/[0.02] px-3 py-2.5 flex items-center justify-between gap-2"
                    >
                      <div className="min-w-0 flex items-center gap-2">
                        <span className="text-[14px] shrink-0">{CAT_ICONS[job.c]}</span>
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <p className="text-white font-semibold text-[12px] leading-snug truncate">{job.t}</p>
                          ...
                          <p className="text-gray-500 text-[10px] truncate">📍 {job.l}{job.sm > 0 ? ` · ${job.s}` : ""}</p>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        <span className="text-emerald-400 font-black text-[10px] whitespace-nowrap">
                          {job.sm > 0 ? job.s : "—"}
                        </span>
                        <ApplyPreScreen
                          waBase={WA_BASE}
                          jobTitle={job.t}
                          source={`vacancies-urgent`}
                          jobId={job.slug}
                          referralMode
                        >
                          {(openFn) => (
                  ...
                  {isOpen && (
                    <div className="flex flex-col gap-1.5 px-3 pb-3">
                      {jobs.map((job) => (
                        ...
                        >
                          <div className="min-w-0 flex items-center gap-2">
                            <span className="text-[14px] shrink-0">{CAT_ICONS[c]}</span>
                            <div className="min-w-0">
                              <p className="text-white font-semibold text-[12px] leading-snug truncate">{job.t}</p>
                              <p className="text-gray-500 text-[10px] truncate">📍 {job.l}{job.sm > 0 ? ` · ${job.s}` : ""}</p>
                            </div>
