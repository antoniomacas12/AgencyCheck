...
// ─── DesktopApplyButton ────────────────────────────────────────────────────────
// Drop-in client component for the "Apply via WhatsApp" CTA on desktop.
// Wraps ApplyPreScreen so the EU + BSN pre-qualification gate is enforced
// on desktop the same way it is on mobile (StickyApplyBar).
//
// Usage on any job page:
//   <DesktopApplyButton waBase={WA_BASE} jobTitle={JOB_TITLE} source="warehouse" jobId="warehouse" />
// ─────────────────────────────────────────────────────────────────────────────
...
export default function DesktopApplyButton({ waBase, jobTitle, source, jobId }: Props) {
  return (
    <ApplyPreScreen waBase={waBase} jobTitle={jobTitle} source={source} jobId={jobId}>
      {(openFn) => (
        <button
          onClick={openFn}
