...
export default function LeadsDashboard() {
  ...
  const [waLeads,        setWaLeads]        = useState<{ count: number; since: string | null } | null>(null);
  ...
  useEffect(() => {
    fetch("/api/prequalification")
      .then((r) => r.json())
      .then((d) => setWaLeads({ count: d.qualified ?? 0, since: d.trackingSince ?? null }))
      .catch(() => {}); // non-critical
  }, []);
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-5">
        <div className="max-w-7xl mx-auto flex flex-col gap-4">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-500 shrink-0 mt-1">
              <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />Live
            </div>
          </div>

          {/* WhatsApp leads counter — full width on mobile */}
          {waLeads !== null && (
            <div className="flex items-center gap-3 bg-[#f0fdf4] border border-[#22c55e]/30 rounded-xl px-4 py-3 w-full">
              <svg className="w-5 h-5 text-[#16a34a] shrink-0" viewBox="0 0 24 24" fill="currentColor">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-wider text-[#16a34a] leading-none mb-0.5">WhatsApp leads</p>
                <p className="text-2xl font-black text-gray-900 leading-none">{waLeads.count}</p>
              </div>
              {waLeads.since && (
                <p className="text-[11px] text-gray-400 shrink-0 text-right">
                  tracking since<br />
                  <span className="font-semibold text-gray-500">
                    {new Date(waLeads.since).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}
                  </span>
                </p>
              )}
            </div>
