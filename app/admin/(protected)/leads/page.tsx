...
const SOURCE_LABELS: Record<LeadSourceType, string> = {
  jobs_with_housing: "Housing page", job_page: "Job page", agency_page: "Agency page", general_apply: "General",
};
...
export default function LeadsDashboard() {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    try {
      ...
      if (q) p.set("q", q); if (statusF) p.set("status", statusF);
      if (accomF) p.set("accommodation", accomF); if (workF) p.set("workType", workF);
          ...
          {(Object.keys(STATUS_META) as LeadStatus[]).map((s) => {
            ...
            return (
              <button key={s} onClick={() => setStatusF(active ? "" : s)}
                className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border transition ${active ? `${m.bg} ${m.text} border-current shadow-sm` : "bg-white text-gray-600 border-gray-200 hover:border-gray-400"}`}>
                <span className={`w-1.5 h-1.5 rounded-full ${m.dot}`} />{m.label}{cnt > 0 && <span className="opacity-70 ml-0.5">({cnt})</span>}
              </button>
            );
          })}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 mb-5 flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[180px]">
            ...
            <div className="relative">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Name, phone, email…" className="w-full pl-8 pr-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <select value={workF} onChange={(e) => setWorkF(e.target.value)} className="text-sm border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none bg-white">
              <option value="">All types</option>{Object.entries(WORK_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
          </div>
          <div>
            ...
            <select value={sourceF} onChange={(e) => setSourceF(e.target.value)} className="text-sm border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none bg-white">
              <option value="">All sources</option>{(Object.keys(SOURCE_LABELS) as LeadSourceType[]).map((s) => <option key={s} value={s}>{SOURCE_LABELS[s]}</option>)}
            </select>
          </div>
          <button onClick={fetchLeads} className="px-4 py-1.5 text-sm font-semibold bg-brand-600 text-white rounded-lg hover:bg-brand-700 transition">Refresh ↻</button>
        </div>
        {(actionError || error) && (
          <div className="bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-2 rounded-lg mb-4 flex items-center justify-between">
            ...
            <button onClick={() => { setActionError(null); setError(null); }} className="text-red-400 hover:text-red-600 text-lg">×</button>
          </div>
        )}
        {/* Table */}
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          {loading ? (
            <div className="py-20 text-center"><div className="w-6 h-6 border-2 border-brand-600 border-t-transparent rounded-full animate-spin mx-auto mb-2" /><p className="text-sm text-gray-400">Loading…</p></div>
          ) : error ? (
            <div className="py-20 text-center space-y-2"><p className="text-2xl">⚠️</p><p className="text-sm font-semibold text-red-600">Could not load leads</p><p className="text-xs text-gray-400">{error}</p><p className="text-xs text-gray-400">Check database connection and migrations.</p></div>
          ) : leads.length === 0 ? (
            <div className="py-20 text-center"><p className="text-3xl mb-3">📭</p><p className="text-sm font-semibold text-gray-700">No leads found</p><p className="text-xs text-gray-400 mt-1">{q || statusF || accomF || workF || sourceF ? "Try clearing filters" : "Apply form submissions will appear here"}</p></div>
          ) : (
                    ...
                    <tr key={lead.id} className={`hover:bg-gray-50/70 transition-colors group ${["new","reviewed","waiting_for_match","potential_fit"].includes(lead.status) ? "bg-blue-50/20" : ""}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {!["approved","sent","converted","rejected"].includes(lead.status) && (
                            <>
                              <button onClick={() => quickAction(lead.id, "approve")} disabled={acting === lead.id} className="px-2 py-1 text-[10px] font-bold bg-green-600 text-white rounded hover:bg-green-700 disabled:opacity-50 transition">✓ Approve</button>
                              <button onClick={() => quickAction(lead.id, "reject")} disabled={acting === lead.id} className="px-2 py-1 text-[10px] font-bold bg-red-500 text-white rounded hover:bg-red-600 disabled:opacity-50 transition">✗ Reject</button>
                            </>
                          )}
        ...
        {pages > 1 && (
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1.5 text-xs font-semibold border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-40 transition">← Prev</button>
              <button onClick={() => setPage((p) => Math.min(pages, p + 1))} disabled={page === pages} className="px-3 py-1.5 text-xs font-semibold border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-40 transition">Next →</button>
            </div>
          </div>
        )}
