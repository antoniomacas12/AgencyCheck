...
export const metadata: Metadata = {
  title: "Forklift & Warehouse Worker Job in Tiel | Apply Now | AgencyCheck",
  description:
    "Forklift operator and warehouse worker vacancies in Tiel, Netherlands. €14.71–€16.50/hour, housing included. EU workers welcome. Apply via WhatsApp — same-day response.",
  alternates: {
    canonical: "https://agencycheck.io/jobs/forklift-warehouse-worker-tiel",
  },
  openGraph: {
    title: "Forklift & Warehouse Worker Job in Tiel | Apply Now",
    description: "€14.71–€16.50/hour + housing included in Tiel. Apply via WhatsApp today.",
  },
};
...
const POSITIONS = [
  {
    ...
    tag: "Forklift cert required",
    icon: "🔧",
    detail: "Counter-balance or reach-forklift operation inside a busy distribution centre. VCA certificate or equivalent required.",
  },
  {
    title: "Warehouse Worker",
    salary: "€14.71/hour",
    tag: "No certificate needed",
    icon: "📦",
    detail: "Order picking, packing, and loading/unloading. No prior experience required — on-site training provided.",
  },
];

const SALARY_ROWS = [
  { label: "Gross pay (€14.71 × 40h/week)",  amount: "+€588", positive: true },
  { label: "Tax & social (loonheffing)",       amount: "−€63",  positive: false },
  ...
  { label: "💶 You keep per week",             amount: "≈€367", positive: true },
] as const;

const BENEFITS = [
  { icon: "🏠", label: "Housing included",      detail: "SNF-inspected shared accommodation. Costs deducted — no deposit." },
  { icon: "🚌", label: "Transport arranged",    detail: "Bus from accommodation to the warehouse. No car needed." },
  { icon: "📄", label: "ABU CAO contract",      detail: "All deductions listed before you travel. Transparent contract." },
  { icon: "🌍", label: "EU workers welcome",    detail: "Polish, Romanian, English communication. No Dutch required." },
  { icon: "⚡", label: "Fast placement",        detail: "Start within 1–2 weeks. WhatsApp reply same working day." },
  { icon: "🆓", label: "Free to apply",         detail: "No placement fee for workers. Ever." },
];

export default function ForkliftWarehouseWorkerTielPage() {
  return (
    <div className="min-h-screen bg-[#0B1F14] text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-6 pb-2">
        <nav className="flex items-center gap-2 text-[12px] text-gray-500">
          ...
          <span>/</span>
          <span className="text-gray-300">Forklift & Warehouse — Tiel</span>
        </nav>
      </div>
      ...
      <section className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 mb-4">
          ...
          <span className="text-[11px] font-semibold tracking-widest uppercase text-emerald-300">2 positions open</span>
        </div>

        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white mb-3 leading-tight">
          Forklift &amp; Warehouse<br />
        ...
        <p className="mt-3 text-[12px] text-gray-500">Free · Same-day WhatsApp response</p>
      </section>

      {/* Position cards */}
      <section className="max-w-3xl mx-auto px-4 sm:px-6 py-8 border-t border-white/[0.06]">
        <h2 className="text-xl font-black text-white mb-4">Open positions</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {POSITIONS.map((pos) => (
            <div key={pos.title} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
              <div className="text-3xl mb-3">{pos.icon}</div>
              <h3 className="text-white font-bold text-[16px] mb-1">{pos.title}</h3>
              <p className="text-emerald-400 font-black text-[18px] mb-2">{pos.salary}</p>
              <span className="inline-block mb-3 text-[11px] font-semibold rounded-full border border-white/10 bg-white/[0.05] px-2.5 py-0.5 text-gray-400">
                {pos.tag}
              </span>
              <p className="text-gray-500 text-[13px] leading-snug">{pos.detail}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Salary breakdown */}
      <section className="max-w-3xl mx-auto px-4 sm:px-6 py-8 border-t border-white/[0.06]">
        <h2 className="text-xl font-black text-white mb-4">Weekly take-home (warehouse worker, 40h)</h2>
        ...
        <p className="text-[11px] text-gray-600 mt-2">Estimate based on 40h/week, ABU CAO Phase A, 2026 tax tables. Forklift operators earn more.</p>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-4 sm:px-6 py-10 border-t border-white/[0.06]">
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.07] px-6 py-8 text-center">
          <p className="text-white font-black text-xl mb-2">Apply Now</p>
          <p className="text-gray-400 text-sm mb-6">
            Send us a WhatsApp message. Tell us which position (forklift or warehouse), whether you have a certificate, and when you can start.
          </p>
          <GateLink
