...
export async function generateMetadata({
  ...
  const canonicalPl = `${CITY_BASE.pl}/${params.city}`;
...
export default async function PlCityPage({
  params,
}: {
  params: { city: string };
}) {
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-8 pb-24">
      ...
      <nav className="text-xs text-gray-400 mb-5 flex items-center gap-1.5 flex-wrap">
        ...
        <span>/</span>
        <Link href="/pl/agencje-pracy-holandia" className="hover:text-brand-600">{S.breadcrumbCities}</Link>
        <span>/</span>
        ...
        <section className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {workerData.agencies.map((a) => (
              <Link
                key={a.agencySlug}
                href={`${AGENCY_BASE.pl}/${a.agencySlug}/${toCitySlug(cityNorm)}`}
                ...
                <div key={c.id} className="flex gap-3 px-4 py-3 border-b border-gray-50 last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mb-1">
                      <Link
                        href={`${AGENCY_BASE.pl}/${c.agencySlug}`}
                    ...
                    <Link
                      href={`${AGENCY_BASE.pl}/${c.agencySlug}/${toCitySlug(cityNorm)}`}
