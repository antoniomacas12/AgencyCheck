    ...
    <div className="min-h-screen bg-[#0B1F14] text-white">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <nav className="text-xs text-gray-400 mb-5 flex items-center gap-1.5">
          ...
          <span>›</span>
          <Link href="/search" className="hover:text-emerald-400">Salary Data</Link>
          <span>›</span>
          <span className="text-gray-300">{job.title} — {city ? city.name : "Netherlands"}</span>
        </nav>
        ...
        <div className="rounded-xl border border-white/[0.07] bg-white/[0.04] p-5 mb-7">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">Min / hr</p>
            </div>
            <div className="border-x border-white/[0.07]">
              <p className="text-3xl font-extrabold text-emerald-400">€{avgPay.toFixed(2)}</p>
              <p className="text-xs text-gray-400 mt-1">Avg / hr reported</p>
            </div>
            <div>
              <p className="text-2xl font-extrabold text-gray-100">€{job.max.toFixed(2)}</p>
              <p className="text-xs text-gray-400 mt-1">Max / hr</p>
            </div>
          </div>

          {/* Monthly estimate */}
          <div className="mt-5 pt-4 border-t border-white/[0.07] grid grid-cols-3 gap-3 text-center text-xs text-gray-400">
            <div>
              <p className="font-semibold text-white text-base">€{(avgPay * 8).toFixed(0)}</p>
              <p>Daily (8hrs)</p>
            </div>
            <div>
              <p className="font-semibold text-white text-base">€{(avgPay * 40).toFixed(0)}</p>
              <p>Weekly (40hrs)</p>
            </div>
            <div>
              <p className="font-semibold text-white text-base">€{(avgPay * 160).toFixed(0)}</p>
              <p>Monthly (160hrs)</p>
            </div>
          </div>
        ...
        {withHousing.length > 0 && (
          <div className="bg-emerald-900/20 border border-emerald-700/30 rounded-xl p-4 mb-7 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <Link
                ...
                className="inline-block mt-2 text-xs font-semibold text-emerald-400 underline hover:text-emerald-300"
              >
              ...
              {["rotterdam", "amsterdam", "eindhoven", "tilburg", "breda", "venlo"].map((cSlug) => {
                ...
                return (
                  <Link
                    ...
                    className="rounded-xl border border-white/[0.07] bg-white/[0.04] p-3 hover:border-emerald-500/30 hover:bg-white/[0.07] transition-all text-sm"
                  >
        ...
        {/* ── Tools CTA ── */}
        <div className="bg-emerald-900/20 border border-emerald-700/30 rounded-xl p-4 mb-8 flex items-center justify-between gap-3 flex-wrap">
          <div>
            <p className="font-semibold text-emerald-200 text-sm">🧮 Calculate your real income</p>
          ...
          <Link
            ...
            className="shrink-0 bg-emerald-500 hover:bg-emerald-400 text-black text-xs font-semibold px-4 py-2 rounded-lg transition-colors"
          >
            Open calculator →
          </Link>
        </div>
