...
export async function generateMetadata({
  ...
  return {
    title,
    description,
    alternates: agencyAlternatesLocale(params.slug, "ro"),
    openGraph: {
      title,
      description,
      locale: "ro_RO",
...
export default async function RoAgencyPage({ params }: { params: { slug: string } }) {
  const staticAgency = ALL_AGENCY_MAP[params.slug];
      ...
      <section className="mb-8">
        ...
        {directReviews.length > 0 ? (
          ...
          <HeroReviewInline
            ...
            beFirst
            labels={{
              heading:        "✍️ Ai lucrat la această agenție? Ajută-i pe alții",
              placeholder:    "Cum a fost cazarea, salariul, managementul?",
              submit:         "Trimite recenzia →",
              successTitle:   "Recenzie trimisă!",
              successSub:     "Mulțumim. Recenzia ta este acum vizibilă pe pagina de recenzii.",
              beFirstHeading: `Fii primul care recenzează ${agencyName}`,
              beFirstSub:     "Muncitori ca tine se bazează pe recenzii sincere pentru a evita agențiile rele. Durează 30 de secunde.",
            }}
          />
        )}
      </section>
