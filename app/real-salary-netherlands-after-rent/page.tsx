...
function calcNetSalary(grossHourly: number, hoursPerWeek: number): {
  ...
  const holidayPay   = grossMonthly * 0.08 / 12;
    ...
    <div className="min-h-screen bg-[#0B1F14] text-white">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <nav className="text-xs text-gray-400 mb-4 flex items-center gap-1.5 flex-wrap">
          ...
          <span>›</span>
          <Link href="/jobs-in-netherlands" className="hover:text-emerald-400">Jobs</Link>
          <span>›</span>
          <span className="text-gray-300 font-medium">Real Salary Calculator</span>
        </nav>

        {/* ── Hero ───────────────────────────────────────────────────────────── */}
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <span className="text-xs bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded-full px-2.5 py-1 font-medium">
              💰 Updated for 2026
            </span>
        ...
        {/* ── Calculator ─────────────────────────────────────────────────────── */}
        <div className="rounded-xl border border-white/[0.07] bg-white/[0.04] p-6 mb-8">
          <h2 className="text-base font-bold text-white mb-5">💰 Salary calculator</h2>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <span className="text-lg font-bold text-emerald-400 w-20 text-right">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <span className="text-lg font-bold text-amber-300 w-20 text-right">
                  €{weeklyRent}/wk
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1 accent-amber-400"
                />
                <span className="text-lg font-bold text-gray-300 w-20 text-right">
                  €{transport}/wk
                </span>
              </div>
            </div>
          ...
          <div className="mt-6 pt-5 border-t border-white/[0.07]">
            ...
            <div className="space-y-2 text-sm">
              {[
                { label: "Gross monthly salary",     value: `€${grossMonthly.toFixed(0)}`,    color: "text-gray-200" },
                { label: "Income tax (estimated)",    value: `− €${taxAmount.toFixed(0)}`,     color: "text-red-400"  },
                { label: "Net monthly salary",        value: `€${netMonthly.toFixed(0)}`,      color: "text-gray-200 font-semibold" },
                { label: "Holiday pay (8% accrued)", value: `+ €${holidayPay.toFixed(0)}`,    color: "text-emerald-400" },
                { label: "Housing deduction",         value: `− €${rentMonthly.toFixed(0)}`,   color: "text-amber-300" },
                { label: "Transport cost",            value: `− €${transportMonthly.toFixed(0)}`, color: "text-amber-300" },
              ].map((row) => (
              ...
              <div className="flex justify-between items-center pt-3 border-t border-white/[0.07] mt-2">
                <span className="font-bold text-white text-base">💵 Monthly take-home</span>
                <span className={`font-extrabold text-xl ${takeHome >= 1200 ? "text-emerald-400" : takeHome >= 900 ? "text-amber-300" : "text-red-400"}`}>
                  €{takeHome.toFixed(0)}
                </span>
              </div>
            </div>
            ...
            {takeHome >= 1200 && (
              <div className="mt-3 p-3 bg-emerald-900/20 border border-emerald-700/30 rounded-lg text-xs text-emerald-300">
        ...
        <section className="mb-8">
          <h2 className="text-base font-bold text-white mb-3">Real-world salary examples (after rent)</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="bg-white/[0.06]">
                  <th className="text-left p-2 border border-white/[0.08] font-semibold text-gray-300">Role</th>
                  <th className="text-left p-2 border border-white/[0.08] font-semibold text-gray-300">Gross/hr</th>
                  <th className="text-left p-2 border border-white/[0.08] font-semibold text-gray-300">Gross/mo</th>
                  <th className="text-left p-2 border border-white/[0.08] font-semibold text-gray-300">After tax</th>
                  <th className="text-left p-2 border border-white/[0.08] font-semibold text-gray-300">After rent (€100/wk)</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={row.role} className="border-b border-white/[0.07]">
                    <td className="p-2 border border-white/[0.08] text-gray-300">{row.role}</td>
                    <td className="p-2 border border-white/[0.08] font-semibold text-white">€{row.hourly.toFixed(2)}</td>
                    <td className="p-2 border border-white/[0.08] text-gray-300">€{row.gross.toLocaleString()}</td>
                    <td className="p-2 border border-white/[0.08] text-blue-300">€{row.net.toLocaleString()}</td>
                    <td className="p-2 border border-white/[0.08] font-bold text-emerald-400">€{row.afterRent.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* ── Key facts ──────────────────────────────────────────────────────── */}
        <section className="mb-8 bg-white/[0.04] border border-white/[0.07] rounded-xl p-5">
          <h2 className="text-sm font-bold text-white mb-3">Key facts about Dutch worker finances</h2>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {[
            ...
            <Link
              key={link.href}
              href={link.href}
              className="inline-flex items-center text-xs font-medium bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded-full px-3 py-1.5 hover:bg-emerald-500/30 transition-colors"
            >
              {link.label}
            </Link>
          ))}
